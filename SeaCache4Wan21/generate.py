#!/usr/bin/env python3
"""Run locked original Wan2.1, optionally injecting clean SeaCache."""
...
def parse_wrapper_args(argv: list[str]) -> tuple[argparse.Namespace, list[str]]:
    ...
    parser.add_argument("--enable_seacache", action="store_true")
    parser.add_argument("--seacache_thresh", type=float)
    parser.add_argument("--seacache_trace", type=Path)
    ...
    sea_options = (
        wrapper_args.seacache_thresh is not None
        or wrapper_args.seacache_trace is not None
        or wrapper_args.use_ret_steps
    )
    if not wrapper_args.enable_seacache and sea_options:
        parser.error("SeaCache options require --enable_seacache")
    if wrapper_args.enable_seacache:
        if wrapper_args.seacache_thresh is None:
            wrapper_args.seacache_thresh = 0.2
        if wrapper_args.seacache_thresh <= 0:
            parser.error("--seacache_thresh must be positive")
...
def main() -> None:
    ...
    if wrapper_args.enable_seacache and args.task != "t2v-1.3B":
        raise ValueError("SeaCache4Wan21 fixed protocol supports only t2v-1.3B.")
    if wrapper_args.enable_seacache and (
        args.t5_fsdp
        or args.dit_fsdp
        ...
        raise ValueError(
            "SeaCache4Wan21 v0.1 supports one GPU without FSDP or sequence parallelism."
        )

    if not wrapper_args.enable_seacache and wrapper_args.timing_json is None:
        ...
        if wrapper_args.enable_seacache:
            ...
            stack.enter_context(
                patch_pipeline_construction(
                    original.wan,
                    task=args.task,
                    threshold=wrapper_args.seacache_thresh,
                    trace_path=(
                        str(wrapper_args.seacache_trace)
                        if wrapper_args.seacache_trace is not None
                        else None
                    ),
        ...
        if wrapper_args.timing_json is not None:
            ...
            stack.enter_context(
                patch_pipeline_timing(
                    ...
                    implementation=(
                        "seacache" if wrapper_args.enable_seacache else "wan21"
                    ),
                )
            )
        original.generate(args)
