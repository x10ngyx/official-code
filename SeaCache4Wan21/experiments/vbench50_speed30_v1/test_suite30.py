...
class Tests(unittest.TestCase):
    def test_calibrated_threshold(self):
        ...
        self.assertAlmostEqual(s.threshold_for(mapping),.4384497621,places=9)
    ...
    def test_sources_parse(self):
        for path in (Path(s.__file__),s.WORKER,Path(__file__).parent/'audit_speed24.py'):
    ...
    def test_full50_finalize(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cfg=dict(previous=str(previous),threshold=.4384497621,shard_ids={})
            comp={}; quality={}
            ...
            self.assertEqual(float(result['target']),3.0)
