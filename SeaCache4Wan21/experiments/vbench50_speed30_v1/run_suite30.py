"""Fixed calibrated SeaCache nominal3.0x on the existing random50, four GPUs."""
...
prior = pair.prior
ROOT = pair.EXP / 'seacache_wan21_vbench50_speed30_4gpu_v1'
WORKER = pair.HERE / 'sea_worker.py'
read, dump, sha, now = pair.read, pair.dump, pair.sha, pair.now


def threshold_for(mapping, target=3.0):
...
def prepare(resume=False):
    ...
    cfg = dict(schema='seacache_random50_speed30_v1', previous=str(previous), protocol=prior.PROTOCOL,
               target=3.0, threshold=threshold, prompt_count=50, gpu_uuids=old['gpu_uuids'],
...
def finalize(cfg):
    ...
    result=dict(method='seacache',target=3.0,threshold=cfg['threshold'],**prior.summarize(base,cand))
    ...
    (out/'RESULTS.md').write_text('# SeaCache 同50prompt名义3.0×\n\n同GPU baseline、固定标定阈值；无VBench或自适应补测。\n\n'
