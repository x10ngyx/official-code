...
def main() -> None:
    ...
    args.seacache_dir = require_external(args.seacache_dir)
    ...
    seacache_paths = timing_paths(args.seacache_dir)
    if set(baseline_paths) != set(seacache_paths):
        missing_candidate = sorted(set(baseline_paths) - set(seacache_paths))
        missing_baseline = sorted(set(seacache_paths) - set(baseline_paths))
        raise ValueError(
            "baseline/SeaCache timing IDs differ: "
    ...
    seacache_summary, seacache_rows = summarize_condition(
        label="seacache",
        paths=seacache_paths,
    ...
    seacache_latency = seacache_summary["end_to_end_inference_latency_seconds"]["mean"]
    ...
    payload = {
        ...
        "comparison": {
            "latency_speedup_baseline_over_seacache": (
                baseline_latency / seacache_latency
                if seacache_latency > 0
                else None
            ),
            "latency_reduction_fraction": (
                1.0 - seacache_latency / baseline_latency
                if baseline_latency > 0
                else None
            ),
            "dit_flops_speedup_ratio_of_sums": (
                baseline_tflops / seacache_tflops if seacache_tflops > 0 else None
            ),
            "dit_flops_reduction_fraction": (
                1.0 - seacache_tflops / baseline_tflops
    ...
    (args.output_dir / "README.md").write_text(
        "# SeaCache4Wan21 performance result\n\n"
