...
def main() -> None:
    ...
    outputs = [root / "benchmark_report.json", root / "benchmark_report.md", root / "benchmark_report.csv"]
    ...
    candidate = performance["conditions"]["seacache"]
    fidelity = {
        "protocol_id": metrics.get("protocol_id"),
        "psnr_rgb_db": metric(metrics, "psnr_rgb_db"),
        "ssim_rgb": metric(metrics, "ssim_rgb"),
        "lpips_alex_v0_1_spatial": metric(metrics, "lpips_alex_v0_1_spatial"),
    }
    payload = {
        "schema": "seacache_vbench200_report_v2",
        "method": "SeaCache4Wan21", "model": "Wan2.1-T2V-1.3B",
        "dataset": "Vbench200", "threshold": config["threshold"],
        ...
        "performance": {
            ...
            "flops_definition": performance["flops_definition"],
            "baseline": {
    ...
    outputs[1].write_text(
        "# SeaCache4Wan21 Vbench200 report\n\n"
    ...
    with outputs[2].open("x", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=["condition", "inference_time_seconds_mean", "estimated_dit_tflops_per_video_mean", "vbench_quality", "vbench_semantic", "vbench_total", "psnr_rgb_db", "ssim_rgb", "lpips_alex_v0_1_spatial"])
        writer.writeheader()
