#!/usr/bin/env python3
"""Validate matched baseline/SeaCache runs and report inference-only speedup."""
...
def validate_timing_payload(payload: dict[str, Any], expected_method: str) -> float:
    ...
    expected_implementation = "seacache" if expected_method == "seacache" else "wan22"
    ...
    for call_index, call in enumerate(calls):
        ...
        if executed not in (0, block_count):
            raise ValueError("SeaCache4Wan22 DiT calls must execute all or zero blocks")
...
def validate_seacache_trace(manifest: dict[str, Any]) -> None:
    trace_path = validate_artifact(manifest["trace"], "SeaCache trace")
    trace = load_json(trace_path)
    if trace.get("schema") != "seacache4wan22_trace_v1":
        raise ValueError("invalid SeaCache trace schema")
    if trace.get("threshold") != manifest["threshold"]:
        raise ValueError("SeaCache trace threshold mismatch")
    if trace.get("use_ret_steps") is not manifest.get("use_ret_steps"):
        raise ValueError("SeaCache trace retention mode mismatch")
    decisions = trace.get("decisions", [])
    if len(decisions) != 50:
        raise ValueError(f"expected 50 SeaCache decisions, found {len(decisions)}")
    if trace.get("total_steps") not in (None, 50):
        raise ValueError("SeaCache trace total-step count mismatch")
    if [row.get("step_index") for row in decisions] != list(range(50)):
        raise ValueError("SeaCache decision indices are incomplete")
    expected_stages = ["high"] * 32 + ["low"] * 18
    if [row.get("stage") for row in decisions] != expected_stages:
        raise ValueError("SeaCache high/low stage path is not 32/18")
    for row in decisions:
        if row.get("action") not in {"recompute", "reuse"}:
            raise ValueError(f"invalid action at step {row.get('step_index')}")
    ...
    for call in timing["calls"]:
        ...
        if call["blocks_executed"] != expected_blocks:
            raise ValueError(
                "SeaCache decision trace and measured block execution disagree at "
...
def validate_manifest(path: Path, expected_method: str) -> tuple[dict[str, Any], float]:
    ...
    if manifest.get("schema") != "seacache4wan22_run_manifest_v1":
    ...
    if not isinstance(manifest.get("use_ret_steps"), bool):
        raise ValueError(f"run manifest has invalid use_ret_steps: {path}")
    if expected_method == "none" and manifest["use_ret_steps"]:
        raise ValueError(f"baseline manifest enables SeaCache retention: {path}")
    if expected_method == "seacache" and not isinstance(threshold, (int, float)):
        raise ValueError(f"SeaCache manifest has invalid threshold: {path}")
    if expected_method == "seacache" and (not math.isfinite(threshold) or threshold <= 0):
        raise ValueError(f"SeaCache manifest threshold must be positive: {path}")
    ...
    if expected_method == "seacache":
        validate_seacache_trace(manifest)
    elif manifest.get("trace") is not None:
        raise ValueError("baseline manifest unexpectedly contains SeaCache artifacts")
...
def main() -> None:
    ...
    parser.add_argument(
        "--seacache-manifest", type=Path, action="append", required=True
    )
    parser.add_argument("--output", type=Path)
    args = parser.parse_args()

    if len(args.baseline_manifest) != len(args.seacache_manifest):
        raise ValueError("baseline and SeaCache manifest counts must match")
    ...
    thresholds = set()
    retention_modes = set()
    for baseline_path, seacache_path in zip(
        args.baseline_manifest, args.seacache_manifest
    ):
        baseline, baseline_seconds = validate_manifest(baseline_path, "none")
        seacache, seacache_seconds = validate_manifest(seacache_path, "seacache")
        if baseline["prompt"] != seacache["prompt"]:
            raise ValueError("matched manifests use different prompts")
        if baseline["protocol"] != seacache["protocol"]:
            raise ValueError("matched manifests use different protocols")
        if baseline["checkpoint"] != seacache["checkpoint"]:
            raise ValueError("matched manifests use different checkpoints")
        if (
            baseline["prepared_source_manifest"]["sha256"]
            != seacache["prepared_source_manifest"]["sha256"]
        ):
            raise ValueError("matched manifests use different prepared source trees")
        thresholds.add(float(seacache["threshold"]))
        retention_modes.add(bool(seacache["use_ret_steps"]))
        baseline_sum += baseline_seconds
        seacache_sum += seacache_seconds
        pairs.append(
            {
                "prompt": baseline["prompt"],
                "baseline_manifest": str(baseline_path.resolve()),
                "seacache_manifest": str(seacache_path.resolve()),
                "baseline_pipeline_generate_wall_seconds": baseline_seconds,
                "seacache_pipeline_generate_wall_seconds": seacache_seconds,
                "speedup": baseline_seconds / seacache_seconds,
            }
        )

    if len(thresholds) != 1:
        raise ValueError("all SeaCache runs in one aggregate must use one threshold")
    if len(retention_modes) != 1:
        raise ValueError("all SeaCache runs in one aggregate must use one retention mode")
    payload = {
        "schema_version": 1,
        "timing_boundary": (
        ...
        "use_ret_steps": next(iter(retention_modes)),
        "pair_count": len(pairs),
        "baseline_pipeline_generate_wall_seconds_sum": baseline_sum,
        "seacache_pipeline_generate_wall_seconds_sum": seacache_sum,
        "inference_only_speedup_ratio_of_sums": baseline_sum / seacache_sum,
        "pairs": pairs,
    }
