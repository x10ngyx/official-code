...
def stage_costs(profile: dict[str, Any]) -> dict[str, dict[str, float]]:
    if profile.get("schema") != "seacache4wan22_calflops_profile_v2" or profile.get("input", {}).get("stage_steps") != STAGE_STEPS:
        raise ValueError("invalid SeaCache4Wan22 Calflops profile")
    costs = {}
...
def candidate_row(
    ...
    if trace.get("schema") != "seacache4wan22_trace_v1" or trace.get("total_steps") != 50:
    ...
    component_latency = extract_component_latency(timing)
    return ({
        "condition": "seacache", "sample_id": sample_id, "timing_path": str(timing_path), "trace_path": str(trace_path),
        "pipeline_generate_wall_seconds": number(timing.get("pipeline_generate_wall_seconds"), str(timing_path), positive=True),
...
def main() -> None:
    ...
    baseline, candidate = summarize("baseline", baseline_rows), summarize("seacache", candidate_rows)
    base_latency, sea_latency = float(baseline["pipeline_generate_wall_seconds"]["total"]), float(candidate["pipeline_generate_wall_seconds"]["total"])
    base_flops, sea_flops = float(baseline["estimated_dit_total_tflops"]), float(candidate["estimated_dit_total_tflops"])
    payload = {
        "schema": "seacache4wan22_vbench200_performance_v2", "threshold": next(iter(thresholds)),
        "protocol": {
