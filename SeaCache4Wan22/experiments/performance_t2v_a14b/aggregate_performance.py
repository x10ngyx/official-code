...
def load_profile(path: Path) -> dict[str, Any]:
    ...
    if profile.get("schema") != "seacache4wan22_calflops_profile_v2":
        raise ValueError("invalid SeaCache4Wan22 Calflops profile schema")
    ...
    *,
    ...
    if expected_method == "seacache":
        ...
        if not isinstance(trace_decisions, list) or len(trace_decisions) != 50:
            raise ValueError(f"invalid SeaCache decisions: {manifest_path}")
    ...
    for call_index, call in enumerate(calls):
        ...
        if blocks_executed not in (0, expected_block_count):
            raise ValueError(
                f"SeaCache4Wan22 supports only all-block or zero-block calls: {call}"
            )
        ...
        if trace_decisions is not None:
            ...
            if action not in {"recompute", "reuse"}:
                raise ValueError(
                    f"invalid SeaCache action at step {step_index}: {action!r}"
                )
            expected_blocks = 0 if action == "reuse" else expected_block_count
            if blocks_executed != expected_blocks:
                raise ValueError(
                    f"DiT block trace disagrees with SeaCache decision at step {step_index}"
                )
    ...
    row = {
        "condition": "baseline" if expected_method == "none" else "seacache",
...
def main() -> None:
    ...
    for baseline_path, seacache_path in zip(
        ...
        seacache, seacache_row = summarize_manifest(
            manifest_path=seacache_path,
            expected_method="seacache",
            profile=profile,
        )
        ...
        if baseline["prepared_source_manifest"]["sha256"] != seacache[
        ...
        seacache_rows.append(seacache_row)
    ...
    seacache_summary = summarize_condition("seacache", seacache_rows)
    baseline_latency = baseline_summary["pipeline_generate_wall_seconds"]["total"]
    seacache_latency = seacache_summary["pipeline_generate_wall_seconds"]["total"]
    baseline_tflops = baseline_summary["estimated_dit_total_tflops"]
    seacache_tflops = seacache_summary["estimated_dit_total_tflops"]
    payload = {
        "schema_version": 2,
        "protocol": {
        ...
        "conditions": {
            ...
            "seacache": seacache_summary,
        },
        "comparison": {
            "latency_speedup_ratio_of_sums": baseline_latency / seacache_latency,
            "latency_reduction_fraction": 1.0 - seacache_latency / baseline_latency,
            "dit_flops_speedup_ratio_of_sums": baseline_tflops / seacache_tflops,
            "dit_flops_reduction_fraction": 1.0 - seacache_tflops / baseline_tflops,
        },
        "warnings": [
            ...
            "SeaCache SEA filter/controller and residual-add FLOPs are excluded to match the TeaCache DiT-only convention; this is not complete method or end-to-end FLOPs.",
        ],
    }
    ...
    with per_video_path.open("x", encoding="utf-8") as handle:
        for row in baseline_rows + seacache_rows:
            handle.write(json.dumps(row, ensure_ascii=False) + "\n")
    readme_path.write_text(
        "# SeaCache4Wan22 performance result\n\n"
