...
def main() -> None:
    ...
    prepared_manifest_path = args.wan22_root / ".seacache4wan22_prepared.json"
    if not prepared_manifest_path.is_file():
        raise FileNotFoundError(
            f"Wan2.2 source is not prepared by SeaCache4Wan22: {args.wan22_root}"
        )
    ...
    if prepared_manifest.get("status") != "pass" or prepared_manifest.get(
        ...
        raise ValueError("invalid SeaCache4Wan22 prepared-source manifest")
    ...
    payload = {
        ...
        "scope": (
            ...
            "the SeaCache SEA filter/controller/residual add remain excluded."
        ),
        "counting_convention": {
            "mac_to_flop": 2,
        ...
        "warnings": [
            ...
            "SeaCache SEA filter/controller and residual-add FLOPs are excluded from the DiT headline and from complete-method claims.",
