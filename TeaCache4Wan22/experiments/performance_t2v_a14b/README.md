...
This folder reproduces the TeaCache4Wan21 latency/Calflops workflow for the
fixed TeaCache4Wan22 protocol.
...
- `profile_calflops.py` independently profiles high/low x cond/uncond DiT
  ...
  analytical dense FlashAttention-core correction to Calflops 0.3.2.
- `aggregate_performance.py` validates matched baseline/TeaCache manifests,
...
linked from `TeaCache4Wan22/experiment_results/`; this source folder contains
...
python aggregate_performance.py \
  ...
  --teacache-manifest /path/to/prompt001_teacache.manifest.json \
...
export, file I/O, and evaluation. T5, DiT, and VAE decode timing are also
stored separately. The Calflops headline covers the DiT forward path; two UMT5
encoder forwards and one VAE decode are profiled and recorded separately.
TeaCache controller/residual operations, scheduler, and export remain excluded,
so the result is not labeled complete end-to-end FLOPs.
