...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--condition", choices=("teacache",), required=True)
...
def main() -> None:
    ...
    if not math.isfinite(args.threshold) or args.threshold <= 0:
        raise ValueError("TeaCache requires a finite --threshold > 0")
    ...
    prepared_manifest = args.wan22_root / ".teacache4wan22_prepared.json"
    ...
    if not args.dry_run:
        ...
        if not readme.exists():
            readme.write_text(
                "# TeaCache4Wan22 Vbench200 generation artifacts\n\n"
                "ID-named videos, inference-only timing traces, TeaCache decision "
    ...
    config = {
        "schema": "teacache4wan22_vbench200_generation_v2", "condition": args.condition,
    ...
    try:
        for ordinal, sample in jobs:
            ...
            required = [video, timing, trace]
            expected_impl = "teacache"
            if any(path.exists() for path in required):
                if args.resume and all(path.is_file() and path.stat().st_size > 0 for path in required):
                    ...
                    if trace_payload.get("schema") != "teacache4wan22_trace_v1" or len(trace_payload.get("decisions", [])) != 50:
                        raise ValueError(f"invalid TeaCache trace: {trace}")
                    skipped += 1
