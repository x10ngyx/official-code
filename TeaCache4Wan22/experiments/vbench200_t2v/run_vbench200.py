#!/usr/bin/env python3
"""Run the complete TeaCache4Wan22 Vbench200 benchmark."""
...
DEFAULT_COEFFICIENTS = (
    PROJECT_DIR / "coefficients" / "wan22_t2v_a14b_50step_dpmpp_nonretention.json"
)
WAN22_COMMIT = "42bf4cfaa384bc21833865abc2f9e6c0e67233dc"
VBENCH_DIMENSION_MODULES = [
    "vbench.subject_consistency", "vbench.background_consistency",
    "vbench.temporal_flickering", "vbench.motion_smoothness",
    "vbench.dynamic_degree", "vbench.aesthetic_quality",
    "vbench.imaging_quality", "vbench.object_class", "vbench.multiple_objects",
    "vbench.human_action", "vbench.color", "vbench.spatial_relationship",
    "vbench.scene", "vbench.appearance_style", "vbench.temporal_style",
...
def check_python(
    ...
    *,
    ...
    result = subprocess.run(
        ...
        text=True,
        env={**os.environ, **THREAD_ENV, **(extra_env or {})},
    )
...
def validate_reusable_baseline(
    source: Path,
    current_prepared_manifest: Path,
    checkpoint_dir: Path,
    expected_count: int,
) -> dict[str, Any]:
    """Validate the SeaCache VBench200 no-cache artifacts for direct reuse."""
    ...
    for path in config_paths:
        ...
        requirements = (
            (payload.get("schema") in {"seacache4wan22_vbench200_generation_v1", "seacache4wan22_vbench200_generation_v2"}, "schema"),
            (payload.get("condition") == "baseline", "condition"),
            (payload.get("threshold") is None, "threshold"),
            (payload.get("use_ret_steps") is False, "retention mode"),
        ...
        checkpoint_paths.add(Path(payload["checkpoint_dir"]).expanduser().resolve(strict=True))
    ...
    if checkpoint_paths != {checkpoint_dir.resolve()}:
        raise ValueError(f"baseline/current checkpoint mismatch: {checkpoint_paths}")
    ...
    for sample_id in sorted(expected_ids):
        video_path = source / "videos" / f"{sample_id}.mp4"
        total_video_bytes += video_path.stat().st_size
        ...
        block_counts = timing.get("transformer_block_count_by_stage")
        if (
            not isinstance(calls, list)
            or len(calls) != 100
            or not isinstance(block_counts, dict)
            or set(block_counts) != {"high", "low"}
            or timing.get("full_compute_forward_calls") != 100
            or timing.get("reuse_forward_calls") != 0
            or any(
                call.get("call_index") != index
                or call.get("step_index") != index // 2
                or call.get("model_stage") != ("high" if index // 2 < 32 else "low")
                or call.get("cfg_branch") != ("cond" if index % 2 == 0 else "uncond")
                or call.get("blocks_executed")
                != block_counts.get("high" if index // 2 < 32 else "low")
                or call.get("full_compute") is not True
                or call.get("reuse") is not False
                for index, call in enumerate(calls)
            )
        ):
            raise ValueError(f"reusable baseline is not a 100-full-call run: {sample_id}")
    ...
    if (
        source_prepared.get("schema") != "seacache4wan22_prepared_v1"
        or source_prepared.get("status") != "pass"
        or source_prepared.get("mode") != "prepared"
        or source_prepared.get("wan22_commit") != WAN22_COMMIT
    ):
        raise ValueError("baseline is not from the locked SeaCache4Wan22 source")
    if (
        current_prepared.get("schema") != "teacache4wan22_prepared_tree_validation_v1"
        or current_prepared.get("status") != "pass"
        or current_prepared.get("mode") != "prepared"
        or current_prepared.get("wan22_commit") != WAN22_COMMIT
    ):
        raise ValueError("candidate is not from the locked TeaCache4Wan22 source")
    return {
        "source": str(source),
        "source_method": "SeaCache4Wan22 no-cache baseline",
        "reuse_mode": "direct_directory_symlink_no_regeneration",
        "artifact_counts": {"videos": len(video_ids), "timings": len(timing_ids)},
        "video_total_bytes": total_video_bytes,
        ...
        "shared_wan22_commit": WAN22_COMMIT,
        "compatibility_check": (
            "same VBench200 prompt manifest, checkpoint, fixed generation protocol, "
            "and upstream Wan2.2 commit; source baseline has cache disabled and all "
            "100 DiT calls execute every Transformer block"
        ),
    }
...
def link_file(link: Path, target: Path) -> None:
    if link.is_symlink() and link.resolve() == target:
        return
    if link.exists() or link.is_symlink():
        raise FileExistsError(f"refusing existing file link path: {link}")
...
def generation_command(args: argparse.Namespace, condition: str, shard: int) -> list[str]:
    if condition != "teacache":
        raise ValueError("the formal TeaCache runner never generates a baseline")
    command = [
...
def candidate_generation_complete(args: argparse.Namespace) -> bool:
    """Validate a finished candidate set so evaluation resume never reloads WanT2V."""
    root = args.output_dir / "teacache"
    if not root.is_dir():
        return False
    prompt_rows = [
        json.loads(line)
    ...
    observed = {
        name: {
            path.stem
            for path in (root / name).glob(f"*.{extension}")
            if path.is_file() and path.stat().st_size > 0
        }
        for name, extension in (("videos", "mp4"), ("timings", "json"), ("traces", "json"))
    }
    if any(ids != expected_ids for ids in observed.values()):
        return False
    coefficient_sha = sha256(args.coefficients)
    for sample_id in sorted(expected_ids):
        timing = json.loads((root / "timings" / f"{sample_id}.json").read_text(encoding="utf-8"))
        trace = json.loads((root / "traces" / f"{sample_id}.json").read_text(encoding="utf-8"))
        lifecycle = timing.get("pipeline_lifecycle")
        if (
            timing.get("status") != "success"
            or timing.get("implementation") != "teacache"
            or timing.get("model_forward_call_count") != 100
            ...
            or trace.get("schema") != "teacache4wan22_trace_v1"
            or trace.get("total_steps") != 50
            or len(trace.get("decisions", [])) != 50
            or trace.get("use_ret_steps") is not False
            or trace.get("coefficients_sha256") != coefficient_sha
            or not math.isclose(float(trace.get("threshold", math.nan)), args.threshold, rel_tol=0.0, abs_tol=1e-12)
        ):
            return False
    configs = sorted(root.glob("generation_config.shard_*.json"))
    statuses = sorted((root / "worker_status").glob("worker_*.json"))
    if len(configs) != len(args.gpu_ids) or len(statuses) != len(args.gpu_ids):
        return False
    for shard, (config_path, status_path) in enumerate(zip(configs, statuses)):
        config = json.loads(config_path.read_text(encoding="utf-8"))
        worker = json.loads(status_path.read_text(encoding="utf-8"))
        if (
            config.get("schema") != "teacache4wan22_vbench200_generation_v2"
            or config.get("shard_index") != shard
            or config.get("num_shards") != len(args.gpu_ids)
            or config.get("runner", {}).get("persistent_pipeline") is not True
            or worker.get("status") != "complete"
            or worker.get("pipeline_initialization_count") != 1
        ):
            return False
    return True
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--calflops-profile-source",
        type=Path,
        help="Reuse one completed locked-protocol Calflops profile.",
    )
    ...
    parser.add_argument(
        "--vbench-pythonpath",
        help="Optional os.pathsep-separated paths containing the locked VBench source/dependencies.",
    )
    parser.add_argument(
        "--reference-vbench-source",
        type=Path,
        help="Reuse an already completed baseline VBench evaluation.",
    )
    ...
    parser.add_argument(
        "--preflight-only",
        action="store_true",
        help="Validate source, baseline, Python dependencies, and metric caches without writing outputs.",
    )
...
def main() -> None:
    ...
    if args.preflight_only and args.dry_run:
        raise ValueError("--preflight-only and --dry-run are mutually exclusive")
    ...
    args.coefficients = args.coefficients.expanduser().resolve(strict=True)
    ...
    if args.reference_vbench_source is not None:
        args.reference_vbench_source = external(args.reference_vbench_source)
    vbench_env = {}
    if args.vbench_pythonpath:
        pythonpath_entries = [
            Path(entry).expanduser().resolve(strict=True)
            for entry in args.vbench_pythonpath.split(os.pathsep)
            if entry
        ]
        if not pythonpath_entries:
            raise ValueError("--vbench-pythonpath contains no paths")
        args.vbench_pythonpath = os.pathsep.join(str(path) for path in pythonpath_entries)
        vbench_env["PYTHONPATH"] = args.vbench_pythonpath
    if args.calflops_source is not None:
        args.calflops_source = args.calflops_source.expanduser().resolve(strict=True)
    if args.calflops_profile_source is not None:
        args.calflops_profile_source = external(args.calflops_profile_source)
        if not args.calflops_profile_source.is_file():
            raise FileNotFoundError(args.calflops_profile_source)
    if not args.dry_run:
        ...
        if not args.skip_evaluation and (not args.defer_evaluation or args.preflight_only):
            check_python(args.video_metrics_python, ["torch", "lpips"], "VideoMetrics")
            check_python(
                args.vbench_python,
                ["torch", "detectron2._C", "vbench", *VBENCH_DIMENSION_MODULES],
    ...
    prepared = args.wan22_root / ".teacache4wan22_prepared.json"
    if not prepared.is_file():
        raise FileNotFoundError(f"Wan2.2 source must be prepared by TeaCache4Wan22: {args.wan22_root}")
    subprocess.run(
        [
            str(args.generation_python),
            str(PROJECT_DIR / "scripts" / "validate_prepared_tree.py"),
            "--source", str(args.wan22_root), "--mode", "prepared",
        ],
        check=True,
        env={**os.environ, **THREAD_ENV},
    ...
    baseline_reuse = validate_reusable_baseline(
        args.baseline_source, prepared, args.checkpoint_dir, args.limit or 200
    )
    config = {
        "schema": "teacache4wan22_vbench200_run_v1", "method": "TeaCache4Wan22",
        "threshold": args.threshold, "use_ret_steps": False,
        "coefficients": {
            "path": str(args.coefficients), "sha256": sha256(args.coefficients),
        },
        "protocol": {
        ...
        "generation_runner": {
            ...
            "baseline": "directly reused SeaCache no-cache artifacts; no regeneration",
        ...
        "paths": {
            ...
            "calflops_profile_source": (
                str(args.calflops_profile_source) if args.calflops_profile_source else None
            ),
            "video_metrics_python": str(args.video_metrics_python), "vbench_python": str(args.vbench_python),
            "vbench_pythonpath": args.vbench_pythonpath,
            "reference_vbench_source": (
                str(args.reference_vbench_source) if args.reference_vbench_source else None
            ),
    ...
    if args.preflight_only:
        print(json.dumps({"status": "pass", "config": config}, ensure_ascii=False, indent=2))
        return
    if args.dry_run:
    ...
    if args.dry_run:
        ...
        status_write(args.output_dir, status)
    if args.resume and not args.dry_run and candidate_generation_complete(args):
        status["phases"]["generation_teacache"] = {
            "status": "complete",
            "resume_validation": "exact candidate video/timing/trace set; no model reload",
        }
        status_write(args.output_dir, status)
    else:
        run_generation(args, "teacache", status)
    profile = args.output_dir / "performance" / "calflops_profile.json"
    if args.calflops_profile_source is not None:
        if not args.dry_run:
            link_file(profile, args.calflops_profile_source)
        else:
            print(f"reuse Calflops profile: {args.calflops_profile_source}")
    elif not (args.resume and profile.is_file()):
        profile_command = [
    ...
    if not (args.resume and performance.is_file()):
        run_logged([
            ...
            "--baseline-dir", str(args.output_dir / "baseline"), "--teacache-dir", str(args.output_dir / "teacache"),
            "--calflops-profile", str(profile), "--output-dir", str(args.output_dir / "performance"),
            "--expected-videos", str(args.limit or 200),
            "--expected-threshold", str(args.threshold),
            "--expected-coefficients-sha256", sha256(args.coefficients),
    ...
    if not args.skip_evaluation:
        ...
        if not (
            ...
            if args.vbench_pythonpath:
                command.extend(["--vbench-pythonpath", args.vbench_pythonpath])
            if args.reference_vbench_source:
                command.extend(["--reference-vbench-source", str(args.reference_vbench_source)])
            if args.resume:
                command.append("--resume")
