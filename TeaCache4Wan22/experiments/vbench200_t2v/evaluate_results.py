...
def main() -> None:
    ...
    parser.add_argument("--vbench-pythonpath")
    parser.add_argument(
        "--reference-vbench-source",
        type=Path,
        help="Reuse one completed baseline VBench evaluation instead of recomputing it.",
    )
    ...
    if args.reference_vbench_source is not None:
        ...
        if not (
            args.reference_vbench_source / "vbench200_aggregate_scores.json"
        ).is_file():
            raise FileNotFoundError(
                "incomplete reusable reference VBench evaluation: "
                f"{args.reference_vbench_source}"
            )
    ...
    if not (args.resume and metrics_summary.is_file()):
        ...
        run([
            ...
            "--reference-dir", str(args.reference_videos), "--candidate-dir", str(args.candidate_videos),
            "--extension", ".mp4", "--expected-frames", str(args.expected_frames), "--device", "cuda:0",
            "--model-cache", str(args.video_metrics_cache_dir), "--output-dir", str(args.output_dir / "video_metrics"),
        ], metrics_env, args.output_dir / "logs" / "video_metrics.log", args.dry_run)
    vbench_env = {
        **base_env, "PYTHON_BIN": str(args.vbench_python), "VBENCH_CACHE_DIR": str(args.vbench_cache_dir),
        "TORCH_HOME": str(args.vbench_cache_dir / "torch"), "HF_HOME": str(args.vbench_cache_dir / "huggingface"),
    ...
    for label, videos in (("reference", args.reference_videos), ("candidate", args.candidate_videos)):
        ...
        if args.resume and (target / "vbench200_aggregate_scores.json").is_file():
            continue
        if label == "reference" and args.reference_vbench_source is not None:
            if target.exists() or target.is_symlink():
                raise FileExistsError(f"refusing existing reference VBench path: {target}")
            if not args.dry_run:
                target.symlink_to(args.reference_vbench_source, target_is_directory=True)
            else:
                print(f"reuse reference VBench: {args.reference_vbench_source}")
            continue
        run(["bash", str(VBENCH / "run_vbench200.sh"), str(videos), str(target), "1"], vbench_env, args.output_dir / "logs" / f"vbench_{label}.log", args.dry_run)
    if not args.dry_run:
        (args.output_dir / "README.md").write_text(
            "# Vbench200 evaluation artifacts\n\n`video_metrics/` uses the repository VideoMetrics fixed protocol; `vbench_reference/` and `vbench_candidate/` are Vbench200 subset scores.\n",
            encoding="utf-8",
        )
