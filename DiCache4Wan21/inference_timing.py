#!/usr/bin/env python3
"""CUDA-synchronized pipeline and DiT instrumentation for DiCache4Wan21."""
...
class _PipelineProfiler:
    ...
    def _finalize_calls(self) -> None:
        for record in self.calls:
            ...
            executed = int(record["blocks_executed"])
            if self.implementation == "dicache" and executed not in {1, self.block_count}:
                raise ValueError(f"DiCache call executed an invalid block count: {executed}")
            if self.implementation != "dicache" and executed != self.block_count:
                raise ValueError(f"baseline call executed an invalid block count: {executed}")
            record["probe_blocks_executed"] = min(executed, 1)
            record["deep_blocks_executed"] = max(executed - 1, 0)
            record["full_compute"] = executed == self.block_count
            record["probe_reuse"] = self.implementation == "dicache" and executed == 1
            record["reuse"] = record["probe_reuse"]
    ...
    def _write(
        ...
        *,
        ...
        payload = {
            ...
            "probe_reuse_forward_calls": sum(
                int(bool(record["probe_reuse"])) for record in self.calls
            ),
            "total_probe_blocks_executed": sum(
                int(record["probe_blocks_executed"]) for record in self.calls
            ),
            "total_deep_blocks_executed": sum(
                int(record["deep_blocks_executed"]) for record in self.calls
            ),
            "calls": self.calls,
            "error": error,
        }
...
def patch_pipeline_timing(
    ...
    *,
    ...
    if task not in {"t2v-1.3B", "t2v-14B"}:
        raise ValueError("DiCache4Wan21 timing supports only Wan2.1 T2V tasks")
