#!/usr/bin/env python3
"""Run locked original Wan2.1, optionally injecting clean DiCache."""
...
def parse_wrapper_args(argv: list[str]) -> tuple[argparse.Namespace, list[str]]:
    ...
    parser.add_argument("--enable_dicache", action="store_true")
    parser.add_argument("--dicache_thresh", type=float)
    parser.add_argument("--dicache_probe_depth", type=int, default=1)
    parser.add_argument("--dicache_dcta", type=parse_bool, default=True)
    parser.add_argument("--retention_ratio", type=float, default=0.2)
    parser.add_argument("--dicache_trace", type=Path)
    parser.add_argument("--timing_json", type=Path)
    wrapper_args, wan_args = parser.parse_known_args(argv)

    cache_options = (
        wrapper_args.dicache_thresh is not None
        or wrapper_args.dicache_trace is not None
    )
    if not wrapper_args.enable_dicache and cache_options:
        parser.error("DiCache options require --enable_dicache")
    if wrapper_args.enable_dicache:
        if wrapper_args.dicache_thresh is None:
            wrapper_args.dicache_thresh = 0.08
        if wrapper_args.dicache_thresh < 0:
            parser.error("--dicache_thresh must be nonnegative")
        if wrapper_args.dicache_probe_depth != 1:
            parser.error("the locked protocol requires --dicache_probe_depth 1")
        if not wrapper_args.dicache_dcta:
            parser.error("the locked protocol requires --dicache_dcta true")
        if not .02 <= wrapper_args.retention_ratio < 1:
            parser.error("--retention_ratio must be in [.02, 1)")
...
def main() -> None:
    ...
    if wrapper_args.dicache_trace is not None:
        external(wrapper_args.dicache_trace)
    if wrapper_args.enable_dicache and args.task != "t2v-1.3B":
        raise ValueError("DiCache4Wan21 fixed protocol supports only t2v-1.3B.")
    if wrapper_args.enable_dicache and (
        args.t5_fsdp
        or args.dit_fsdp
        ...
        raise ValueError(
            "DiCache4Wan21 v0.1 supports one GPU without FSDP or sequence parallelism."
        )
        ...
        if wrapper_args.enable_dicache:
            ...
            stack.enter_context(
                patch_pipeline_construction(
                    original.wan,
                    task=args.task,
                    threshold=wrapper_args.dicache_thresh,
                    probe_depth=wrapper_args.dicache_probe_depth,
                    retention_ratio=wrapper_args.retention_ratio,
                    dcta=wrapper_args.dicache_dcta,
                    trace_path=(
                        str(wrapper_args.dicache_trace)
                        if wrapper_args.dicache_trace is not None
                        else None
                    ),
                )
        ...
        if wrapper_args.timing_json is not None:
            ...
            stack.enter_context(
                patch_pipeline_timing(
                    ...
                    implementation=(
                        "dicache" if wrapper_args.enable_dicache else "wan21"
                    ),
                )
            )
