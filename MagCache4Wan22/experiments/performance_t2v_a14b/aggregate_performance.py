#!/usr/bin/env python3
"""Validate measured calls and aggregate matching baseline/MagCache components."""
...
def summarize_manifest(path, mode, profile):
    ...
    if manifest.get("schema") != "magcache4wan22_run_v1" or manifest.get("status") != "complete" or manifest["mode"] != mode:
    ...
    if mode == "magcache":
        ...
        if trace.get("schema") != "magcache4wan22_trace_v1" or trace["method"] != manifest["method"] or len(trace["calls"]) != 100:
            raise ValueError("invalid MagCache decision trace")
    dit_flops = cuda = 0.0
    reuse = 0
    for i, call in enumerate(calls):
        ...
        if executed not in (0, count) or (mode == "baseline" and executed != count):
            raise ValueError("unexpected partial/full block path")
        if call["reuse"] != (executed == 0) or call["full_compute"] != (executed == count):
        ...
        if trace:
            ...
            if decision["action"] != ("reuse" if executed == 0 else "recompute"):
                raise ValueError("trace action mismatch")
        branch_profile = profile["stages"][stage]["branches"][branch]
        flops = float(branch_profile["estimated_full_flops" if executed else "estimated_always_on_flops"])
...
def validate_profile(profile):
    if profile.get("schema") != "magcache4wan22_calflops_profile_v2":
...
def aggregate(baseline_paths, candidate_paths, profile):
    ...
    for baseline, candidate in zip(baseline_paths, candidate_paths):
        ...
        c, cr = summarize_manifest(candidate, "magcache", profile)
        ...
        method = c["method"]
        rows.append(dict(baseline=br, magcache=cr))
    sums = {mode: {key: sum(row[mode][key] for row in rows)
                   for key in rows[0][mode] if key != "sample_id"}
            for mode in ("baseline", "magcache")}
    return dict(schema="magcache4wan22_performance_v1", pair_count=len(rows), method=method,
                ...
                scope="Complete generate latency; DiT TFLOPs headline and separate T5/VAE counts. MagCache scalar controller, residual addition, CFG/scheduler/export excluded from FLOPs.")


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--baseline-manifest", type=Path, action="append", required=True)
    parser.add_argument("--magcache-manifest", type=Path, action="append", required=True)
    parser.add_argument("--calflops-profile", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    check_environment()
    payload = aggregate(args.baseline_manifest, args.magcache_manifest, read_json(args.calflops_profile))
    ...
    if not (destination.parent / "README.md").exists():
        (destination.parent / "README.md").write_text("# MagCache performance aggregate\n\nValidated paired time and component FLOPs with per-video records.\n")
