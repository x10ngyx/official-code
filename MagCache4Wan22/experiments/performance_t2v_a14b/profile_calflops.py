...
class WanForwardProfile(nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad(), torch.autocast("cuda", dtype=torch.bfloat16), official_forward(
            [self.wan_model], core=self.core, args=method_args(threshold=0), split_steps=32
        ):
...
def main() -> None:
    ...
    prepared_manifest_path = args.wan22_root / ".magcache4wan22_prepared.json"
    if not prepared_manifest_path.is_file():
        raise FileNotFoundError(
            f"Wan2.2 source is not prepared by MagCache4Wan22: {args.wan22_root}"
        )
    ...
    if prepared_manifest.get("status") != "pass" or prepared_manifest.get(
        ...
        raise ValueError("invalid MagCache4Wan22 prepared-source manifest")
    ...
    payload = {
        "schema": "magcache4wan22_calflops_profile_v2",
        "schema_version": 2,
        "tool": {
            "name": "calflops",
            **calflops_metadata,
        ...
        "scope": (
            ...
            "the MagCache controller/residual add remain excluded."
        ),
        "counting_convention": {
            "mac_to_flop": 2,
        ...
        "warnings": [
            ...
            "MagCache controller and residual-add FLOPs are excluded from the DiT headline and from complete-method claims.",
    ...
    if not readme.exists():
        readme.write_text("# MagCache component FLOPs profile\n\nReal-shape high/low full/always-on DiT and T5/VAE Calflops records.\n")
