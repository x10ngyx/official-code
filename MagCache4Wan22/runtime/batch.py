    ...
    try:
        ...
        for row, condition, expected, directory in pending:
            ...
            if condition["mode"] == "magcache":
                actual = [c["action"] for c in read_json(manifest["trace"]["path"])["calls"]]
                if actual != condition["schedule"]:
                    raise ValueError("measured trace disagrees with original gate plan")
