"""Fixed model protocol, output layout, and provenance for MagCache4Wan22."""
...
def validate_source(source):
    ...
    for relative, digest in lock["package_file_sha256"].items():
        if sha256(PROJECT / relative) != digest:
            raise ValueError(f"MagCache package file mismatch: {relative}")
    return dict(schema="magcache4wan22_prepared_v1", status="pass", mode="prepared",
                source=str(source), wan22_commit=commit,
                magcache_commit=lock["magcache"]["commit"],
