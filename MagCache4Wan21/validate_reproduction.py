#!/usr/bin/env python3
"""Validate MagCache4Wan21 source boundaries and optional Wan2.1 checkout."""
...
def validate_magcache(root: Path) -> dict[str, object]:
    root = root.expanduser().resolve()
    expected = LOCK["magcache_method_reference"]
    if git(root, "rev-parse", "HEAD") != expected["commit"]:
        raise ValueError("MagCache commit does not match upstream_lock.json")
    if git(root, "status", "--porcelain", "--untracked-files=no"):
        raise ValueError("MagCache checkout contains tracked modifications")
    observed = {}
    for relative, digest in expected["reference_files"].items():
        observed[relative] = sha256(root / relative)
        if observed[relative] != digest:
            raise ValueError(f"MagCache hash mismatch: {relative}")
    return {"root": str(root), "commit": expected["commit"], "sha256": observed}
...
def main() -> None:
    ...
    parser.add_argument("--magcache-root", type=Path)
    ...
    if args.magcache_root:
        report["magcache"] = validate_magcache(args.magcache_root)
