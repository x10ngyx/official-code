#!/usr/bin/env python3
"""Run the three fixed DiCache VBench200 candidates with one reused baseline."""
...
DEFAULT_PROFILE = Path(
    "/all/yiran07-disk3/huteng_data/exp/dicache4wan22_r02_threshold_scan_gpu123_20260907_194213/round_00/profile/calflops.json"
)
DEFAULT_BASELINE_AUDIT = Path(
    "/all/yiran07-disk3/huteng_data/exp/dicache4wan22_baseline_reuse_audit_20260908/baseline_reuse_manifest.json"
)
TARGETS = (
    dict(label="1p8x", target_speedup=1.8, gpu="1", threshold=.075, retention_ratio=.2),
    dict(label="2p4x", target_speedup=2.4, gpu="2", threshold=.072, retention_ratio=.2),
    dict(label="3p0x", target_speedup=3.0, gpu="3", threshold=.396, retention_ratio=.2),
)
...
def validate_baseline(audit_path: Path, prompts: list[dict], profile: dict) -> tuple[dict, dict[str, dict]]:
    ...
    reported_list = jsonl(reported_path)
    reported_rows = {row["sample_id"]: row for row in reported_list}
    if len(reported_rows) != len(prompts) or len(reported_list) != len(prompts):
    ...
    for prompt in prompts:
        ...
        for key, value in baseline_metric_row(timing).items():
            if key != "sample_id" and (not math.isfinite(value) or value < 0):
                raise ValueError(f"invalid baseline metric: {sample_id}/{key}")

    evidence = audit.get("dicache_scan_comparison", {})
    if evidence.get("compared") != 44 or evidence.get("byte_identical") != 44:
        raise ValueError("missing DiCache four-round baseline equality evidence")
    for record in evidence["records"]:
        for field in ("manifest", "video"):
            item = record[field]
            if sha256(item["path"]) != item["sha256"]:
                raise ValueError("scan baseline equality evidence changed")
    # The reused component FLOPs must agree with the DiCache Calflops profile.
    expected_dit = sum(count * sum(profile["stages"][stage]["branches"][branch]["estimated_full_flops"]
                                   for branch in ("cond", "uncond"))
                       for stage, count in (("high", 32), ("low", 18))) / 1e12
    for reported in reported_rows.values():
        for field, expected in (
                ("estimated_t5_tflops_per_video", profile["component_profiles"]["t5"]["estimated_tflops_per_video"]),
                ("estimated_vae_decode_tflops_per_video", profile["component_profiles"]["vae_decode"]["estimated_tflops_per_video"]),
                ("estimated_dit_tflops", expected_dit)):
            if not math.isclose(float(reported[field]), float(expected), rel_tol=1e-12):
                raise ValueError(f"baseline/profile FLOPs mismatch: {field}")
    provenance = {
...
def aggregate_reused(root: Path, plan: dict, profile: dict, reported: dict[str, dict]) -> dict:
    ...
    for prompt in plan["prompts"]:
        ...
        _, candidate = summarize_manifest(path, "dicache", profile)
        ...
        rows.append({"baseline": baseline, "dicache": candidate})
    ...
    result = {
        "schema": "dicache4wan22_performance_v1",
        "pair_count": len(rows),
        "method": method,
        "requested_target_speedup": plan["requested_target_speedup"],
...
def make_plan(args: argparse.Namespace, target: dict) -> tuple[dict, dict[str, dict]]:
    namespace = argparse.Namespace(
        ...
        retention_ratio=target["retention_ratio"],
    )
    plan = build_plan(namespace, False)
    plan["conditions"] = plan["conditions"][1:]
    plan.update(
        schema="dicache4wan22_reused_baseline_batch_plan_v1",
    ...
    if profile["source"]["prepared_manifest"] != plan["source"]:
        raise ValueError("profile/core source mismatch")
    if Path(read_json(args.baseline_audit)["checkpoint"]).resolve() != Path(plan["checkpoint"]).resolve():
        raise ValueError("baseline/candidate checkpoint mismatch")
    plan["baseline_reuse"] = provenance
    plan["experiment_source"] = {name: artifact(SCRIPT_DIR / name)
                                for name in ("run_gpu123.py", "run_gpu123.sh", "audit_baseline.py")}
    plan["target_label_note"] = "User-requested labels; thresholds .075/.072/.396 are explicit user settings, not guaranteed measured speeds."
...
def run_target(args: argparse.Namespace, target: dict) -> None:
    ...
    with (root / ".runner.lock").open("a") as lock:
        ...
        try:
            ...
            evaluation_plan = dict(plan, conditions=[dict(id="baseline", mode="baseline")] + plan["conditions"])
            quality = evaluate(root, evaluation_plan)
            report = {
                "schema": "dicache4wan22_reused_baseline_batch_report_v1",
                "status": "complete", "plan": plan,
                "performance": {plan["conditions"][0]["id"]: result}, **quality,
                "requested_target_speedup": target["target_speedup"],
                "measured_speedup": result["speedup"],
                "relative_error_to_requested_target": result["speedup"] / target["target_speedup"] - 1,
            }
...
def parent(args: argparse.Namespace) -> None:
    ...
    index_result(root)
    (root / "README.md").write_text(
        "# DiCache4Wan22 VBench200 three-target run\n\n"
    ...
    launch = {
        "schema": "dicache4wan22_vbench200_gpu123_launch_v1",
    ...
    try:
        for target in TARGETS:
            ...
            processes.append((target, process, log, log_path))
            # Keep at most one model loader active to limit host RAM and disk contention.
            marker = root / f"target_{target['label']}_gpu{target['gpu']}" / "worker_status/worker_000.json"
            deadline = time.monotonic() + args.worker_ready_timeout
            while True:
                if process.poll() is not None:
                    if process.returncode != 0:
                        raise RuntimeError(f"controller failed: {log_path}")
                    break
                if marker.exists():
                    ready = read_json(marker)
                    if ready.get("status") in ("ready", "running", "complete"):
                        try:
                ...
                if time.monotonic() > deadline:
                    raise TimeoutError(f"target initialization timeout: {target['label']}")
                time.sleep(2)
...
if __name__ == "__main__":
    ...
    if parsed.target_only:
        ...
        root = external_output(parsed.output_dir)
        if parsed.dry_run:
            parent(parsed)
        else:
            if root.exists() and not parsed.resume:
                raise FileExistsError("existing output requires --resume")
            root.mkdir(parents=True, exist_ok=True)
