...
from common import check_environment, checkpoint_path, validate_source, index_result
...
def profile_stage(
    *,
    ...
    for branch in ("cond", "uncond"):
        ...
        model.blocks = nn.ModuleList(list(original_blocks)[:1])
        try:
            probe_calflops = profile_case(wrapper, inputs, calculate_flops_fn)
        finally:
        ...
        branch_profiles[branch] = {
            ...
            "calflops_probe": probe_calflops,
            "probe_depth": 1,
            "estimated_probe_flops": probe_calflops["flops"] + attention_correction / block_count,
            "estimated_probe_tflops": (probe_calflops["flops"] + attention_correction / block_count) / TFLOP_DIVISOR,
    ...
    for key in ("estimated_full_flops", "estimated_always_on_flops", "estimated_probe_flops"):
...
def main() -> None:
    ...
    args.checkpoint_dir = checkpoint_path(args.checkpoint_dir)
    ...
    prepared_manifest_path = args.wan22_root / ".dicache4wan22_prepared.json"
    if not prepared_manifest_path.is_file():
        raise FileNotFoundError(
            f"Wan2.2 source is not prepared by DiCache4Wan22: {args.wan22_root}"
        )
    prepared_manifest = validate_source(args.wan22_root)
    if json.loads(prepared_manifest_path.read_text()) != prepared_manifest:
        raise ValueError("stale prepared manifest; re-prepare the source with this package")
    if prepared_manifest.get("status") != "pass" or prepared_manifest.get(
        "mode"
    ) != "prepared":
        raise ValueError("invalid DiCache4Wan22 prepared-source manifest")
    ...
    payload = {
        "schema": "dicache4wan22_calflops_profile_v2",
        "schema_version": 2,
        "tool": {
            "name": "calflops",
            **calflops_metadata,
        ...
        "scope": (
            ...
            "the DiCache gate/DCTA/residual add remain excluded; reuse includes one full probe block."
        ),
        "counting_convention": {
            "mac_to_flop": 2,
        ...
        "warnings": [
            ...
            "DiCache gate/DCTA and residual-add FLOPs are excluded from the DiT headline and from complete-method claims.",
            "TFLOP/s derived later is achieved estimated DiT throughput, not vendor peak throughput.",
        ],
    }
    args.output.parent.mkdir(parents=True, exist_ok=True)
    index_result(args.output.parent)
    readme = args.output.parent / "README.md"
    if not readme.exists():
        readme.write_text("# DiCache component FLOPs profile\n\nReal-shape high/low full/probe/always-on DiT and T5/VAE Calflops records.\n")
