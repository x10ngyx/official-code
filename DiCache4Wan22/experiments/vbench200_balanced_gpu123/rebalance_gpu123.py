...
sys.path.insert(0, str(PROJECT / "experiments/vbench200_reuse_gpu123"))
...
def target_context(root: Path) -> dict[str, dict]:
    ...
    for name in TARGET_DIRS:
        ...
        if len(plan["conditions"]) != 1 or plan["conditions"][0]["mode"] != "dicache":
...
def validate_any_gpu(path: Path, context: dict) -> tuple[dict, dict]:
    ...
    manifest, row = summarize_manifest(path, "dicache", profile)
...
def completed_and_means(contexts: dict[str, dict]) -> tuple[dict[str, set[str]], dict[str, float]]:
    ...
    for name, context in contexts.items():
        ...
        completed[name] = ids
        # New .172 target has no reusable samples: use the nearby .175 scan cost.
        means[name] = sum(values) / len(values) if values else float(context["plan"]["estimated_generate_seconds"])
...
def build_assignment(root: Path, contexts: dict[str, dict]) -> dict:
    ...
    return {
        "schema": "dicache4wan22_balanced_resume_assignment_v1",
        ...
        "experiment_source": {p.name: artifact(p) for p in sorted(SCRIPT_DIR.glob("*.py"))},
    }
...
def worker(root: Path, gpu: str, resume: bool) -> None:
    ...
    for record in assignment["experiment_source"].values():
        if sha256(record["path"]) != record["sha256"]:
            raise ValueError("balanced experiment source changed")
...
def aggregate_target(context: dict, reported: dict[str, dict]) -> dict:
    ...
    value = {
        "schema": "dicache4wan22_performance_v1", "pair_count": len(rows), "method": method,
    ...
    with (target / ".runner.lock").open("a") as lock:
        ...
        try:
            ...
            evaluation_plan = dict(plan, gpu_ids=[quality_gpu],
                                   conditions=[dict(id="baseline", mode="baseline")] + plan["conditions"])
            ...
            report = {
                ...
                "relative_error_to_requested_target": result["speedup"] / plan["requested_target_speedup"] - 1,
            }
    ...
    with (root / ".balanced_runner.lock").open("a") as lock:
        ...
        try:
            ...
            for gpu in GPUS:
                ...
                # Stagger expensive model loading; warmup can overlap next loader.
                deadline = time.monotonic() + 1800
                marker = root / "balanced_worker_status" / f"gpu_{gpu}.json"
                while True:
                    if any(p.poll() not in (None, 0) for _, p, _, _ in records):
                        wait_processes(records)
                    if marker.exists():
                        state = read_json(marker)
                        if state.get("pid") == process.pid and state.get("status") in ("ready", "running", "complete"):
                            break
                    if process.poll() is not None or time.monotonic() >= deadline:
                        for _, p, _, _ in records:
                            if p.poll() is None:
                                p.terminate()
                        raise RuntimeError(f"worker readiness failed: {path}")
                    time.sleep(2)
            wait_processes(records)
