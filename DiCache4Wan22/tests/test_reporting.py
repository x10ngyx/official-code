...
def fixture(root):
    ...
    source = {"wan22_commit": "fixture", "dicache_commit": "fixture"}
    profile = dict(schema="dicache4wan22_calflops_profile_v2",
                   ...
                   stages={stage: dict(model=dict(transformer_blocks=40),
                                       branches={branch: dict(estimated_full_flops=10e12,
                                                              estimated_always_on_flops=1e12, estimated_probe_flops=2e12, probe_depth=1)
                                                 for branch in ("cond", "uncond")})
                           for stage in ("high", "low")})
    paths = []
    for mode, seconds in (("baseline", 200.), ("dicache", 100.)):
        calls = []
        for i in range(100):
            # Unequal CFG decisions catch accidental step-level reuse averaging.
            reused = mode == "dicache" and i % 2 == 1
            calls.append(dict(call_index=i, step_index=i // 2, model_stage="high" if i < 64 else "low",
                              cfg_branch="cond" if i % 2 == 0 else "uncond", blocks_executed=1 if reused else 40,
        ...
        run = dict(schema="dicache4wan22_run_v1", mode=mode, prompt="fixture prompt", sample_id="one",
                   checkpoint=str(root), protocol=protocol, method=dict(threshold=.08, retention_ratio=.2, probe_depth=1), source=source)
        video = root / (mode + ".mp4")
        ...
        if mode == "dicache":
            trace = dict(schema="dicache4wan22_trace_v1", method=run["method"],
...
class ReportingTests(unittest.TestCase):
    def test_component_and_independent_cfg_flops(self):
        ...
        self.assertEqual(result["sums"]["dicache"]["estimated_dit_tflops"], 600.)
        self.assertEqual(result["sums"]["dicache"]["estimated_t5_tflops_per_video"], 2.)
        self.assertEqual(result["sums"]["dicache"]["estimated_vae_decode_tflops_per_video"], 3.)
    ...
    def test_missing_components_and_duplicate_pairs_fail(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            timing_path = root / "dicache.timing.json"
