#!/usr/bin/env python3
"""CUDA-synchronized pipeline and DiT instrumentation for DiCache4Wan22."""
...
class _PipelineProfiler:
    ...
    def _finalize_calls(self) -> None:
        for record in self.calls:
            ...
            probe = 1 if self.implementation == "dicache" else 0
            executed = record["blocks_executed"]
            if executed not in (probe, block_count):
                raise ValueError("unexpected DiCache probe/full block execution")
            record["reuse"] = executed == probe
            record["probe_blocks_executed"] = min(probe, executed)
...
def patch_pipeline_timing(
    ...
    *,
    ...
    if task != "t2v-A14B":
        raise ValueError("DiCache4Wan22 timing supports only t2v-A14B")
