...
def parser(scan=False):
    p = argparse.ArgumentParser(description="Persistent DiCache parameter scan" if scan else "Persistent DiCache paired batch")
    ...
    if scan:
        ...
        p.add_argument("--target-tolerance", type=float, help="absolute measured speedup tolerance")
    else:
...
def build_plan(args, scan):
    ...
    if scan:
        conditions = grid_conditions(args.thresholds or [.04, .08, .12, .2, .3, .5],
                                     args.retention_ratios or [.2])
    else:
        conditions = grid_conditions([args.threshold], [args.retention_ratio])
    baseline = dict(id="baseline", mode="baseline", threshold=.08, retention_ratio=.2)
    ...
    plan = dict(schema="dicache4wan22_batch_plan_v1", kind="parameter_scan" if scan else "paired_batch",
...
def measured_performance(root, plan, profile):
    ...
    if plan["target_speedups"]:
        selection = select_targets(performance, plan["target_speedups"], plan["target_tolerance"])
...
def main(scan=False, argv=None):
    ...
    with (root / ".runner.lock").open("a") as lock:
        ...
        try:
            ...
            report = dict(schema="dicache4wan22_batch_report_v1", status="complete", plan=plan,
