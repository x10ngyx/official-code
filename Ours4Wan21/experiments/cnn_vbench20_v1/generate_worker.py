...
def generate_jobs(run, jobs_path):
    ...
    if any(j['kind']!='candidate' for j in jobs):raise ValueError('CNN evaluator reuses native baselines; candidate jobs only')
    ...
    for job in jobs:
        ...
        audit_trace(trace,timing,job['skip_budget'])
        overhead={}
        if policy:
        ...
        if controller:torch.save(torch.stack(controller.saved_features),out/'latent_features.pt')
        ...
        seal(out,['identity.json','generation.json','video.mp4','trace.json','measurement.json','timing.json','ffprobe.json','latent_features.pt'],identity=identity)
