...
from dispatch import ROOT,verify_overlap,live_jobs
import argparse
import fcntl
import json
import math
...
def generate_jobs(run, jobs_path, expected):
    require_environment()
    dispatch_config=verify_overlap()
    import torch
    ...
    if not jobs:raise ValueError('need initial ready jobs for native warmup')
    ...
    # Serialize CPU loading, then let native warmups run on separate GPUs.
    ...
    policies={}
    for job in live_jobs(jobs_path,expected):
        ...
        dump(out/'generation.json',dict(identity=identity,gpu_name=prop.name,gpu_uuid=uuid,
            dispatch_source_sha256=dispatch_config['source_hashes'],
...
if __name__=="__main__":
    p=argparse.ArgumentParser();p.add_argument("--jobs",type=Path,required=True);p.add_argument("--expected",type=int,required=True);a=p.parse_args()
    generate_jobs(ROOT,a.jobs,a.expected)
