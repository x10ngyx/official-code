...
- `component_flops.py` profiles fixed-shape UMT5 encoder forwards and Wan VAE
  decode with the locked Calflops implementation and the auditable compatibility
  hook described below.
- `calflops_loader.py` keeps Calflops at version 0.3.2 and installs a scoped
  compatibility hook for its broken tuple `scale_factor` upsample counter;
  interpolation is counted as one FLOP per output element and the patch name
  is recorded in profile provenance.
